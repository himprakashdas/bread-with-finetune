...
wandb.init(project = "bread_finetuning", entity="himprakash-das", config={
    "learning_rate" : 0.01,
    "num_workers": 12,
    "optim":"adam",
    "model":"IAN",
...
def get_args():
    ...
    parser.add_argument('--finetune', action='store_true', help='Whether to perform fine-tuning')
    parser.add_argument('--pretrained_model_path', type=str, default='', help='Path to pre-trained model for fine-tuning')
...
def train(opt):
    ...
    if opt.num_gpus > 0:
        ...
        if opt.num_gpus > 1:
            model = model.cuda()
            # model = DistributedDataParallel(model, device_ids=[0,1,2], output_device=0) ##DEBUG
        ...
        for epoch in range(opt.num_epochs):
            ...
            for iter, (data, target, name) in enumerate(progress_bar):
                ...
                try:
                    ...
                    optimizer.zero_grad()
                    # print("Ekbar dekh lo",data.shape) ##DEBUG
                    # print("Ekbar dekh lo target ",target.shape) ##DEBUG
...
def finetune(opt):
    ...
    if opt.pretrained_model_path:
        pretrained_model = ModelINet(model=models.IAN)
    ...
    model = ModelINet(model=models.IAN)

    # Assuming the same model architecture is used
    # If a pre-trained model is provided, copy weights from it to the new model
    if pretrained_model:
        model.load_state_dict(pretrained_model.state_dict())

    # Modify any additional layers if needed
